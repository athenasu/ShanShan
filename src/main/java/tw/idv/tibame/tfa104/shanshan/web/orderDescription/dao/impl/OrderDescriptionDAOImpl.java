...
//	查詢 特定訂單的訂單明細BO 按訂單明細編號 正序 (沒有圖片)
	private static final String BO_FIND_BY_ORDERID_NO_PIC = "SELECT b.order_id, b.member_id, b.order_created_date, b.order_member_address, b.order_member_name, b.order_member_phone, b.order_status, a.product_quantity, a.product_price, b.order_sum_after, b.order_shipped_date, b.ship_number, b.payment_status, d.product_name, c.product_id, c.product_des_id, c.product_size, c.product_color, e.company_id, e.company_name FROM order_description a JOIN `order` b USING (order_id) JOIN product_description c USING (product_des_id) JOIN company e USING (company_id) JOIN product d USING (product_id) WHERE order_id = ?";
