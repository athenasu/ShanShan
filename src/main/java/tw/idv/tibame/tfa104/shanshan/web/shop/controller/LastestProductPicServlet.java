...
@WebServlet("/lastestProductPicServlet")
@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class LastestProductPicServlet extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		//來源跳轉路徑"<%=contextPath%>/LastestProductPicServlet?productId=${ProductBO.productId}&action=getPopImage"

		response.setContentType("application/json;charset=utf-8");
		request.setCharacterEncoding("UTF-8");

		System.out.println("lastestProductPicServlet被調用了");
